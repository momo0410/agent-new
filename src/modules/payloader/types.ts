...
export interface PayloaderState {
  isLoading: boolean;
  error: string | null;
  webPayloads: PayloadItem[];
  intranetPayloads: PayloadItem[];
  toolCommands: ToolCommand[];
  selectedCategory: string;
  searchQuery: string;
  isInitialized: boolean;
  currentView: 'web' | 'intranet' | 'tools' | 'encoding';
  viewMode: 'list' | 'detail';
  selectedItemId: string | null;
}

export interface PayloaderConfig {
  encoding?: string;
  compression?: boolean;
  chunkSize?: number;
  autoRefresh?: boolean;
...
export interface UsePayloaderReturn {
  state: Readonly<PayloaderState>;
  filteredPayloads: PayloadItem[];
  categories: { id: string; label: I18nText }[];
  isLoading: boolean;
  error: string | null;
  ...
  setCurrentView: (view: 'web' | 'intranet' | 'tools' | 'encoding') => void;
  copyPayload: (code: string) => Promise<boolean>;
  usePayload: (payload: PayloadItem) => void;
  reset: () => void;
  destroy: () => void;
}
